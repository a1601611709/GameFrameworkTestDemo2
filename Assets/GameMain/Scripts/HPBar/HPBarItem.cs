...
namespace StarForceDemo
{
    public class HPBarItem : MonoBehaviour
    {
        ...
        public bool Refresh()
        {
            ...
            if(m_Owner != null && Owner.Logic.Available && Owner.Id == m_OwnerId)
            {
                Vector3 worldPosition = m_Owner.Logic.CachedTransform.position + Vector3.forward;
        ...
        private void Awake()
        {
            ...
            {
                Log.Error("CanvasGroup is invaild");
                return;
            }
        }
